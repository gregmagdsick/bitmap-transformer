...
const transform = function(conversion) {
  if (conversion === 'invert') {
      for (var i = 54; i < bmpHeader.pixelStart; i = i + 4) {
        var aVal = bmp.readUInt8(i);
        var bVal = bmp.readUInt8(i + 1);
        var gVal = bmp.readUInt8(i + 2);
        var rVal = bmp.readUInt8(i + 3);

        bmp.writeUInt8(-(aVal - 255), i);
        bmp.writeUInt8(-(bVal - 255), i + 1);
        bmp.writeUInt8(-(gVal - 255), i + 2);
        bmp.writeUInt8(-(rVal - 255), i + 3);
      }
